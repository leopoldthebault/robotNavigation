...
function[] = nav_plot(W_x,W_y,x_obs,r_obs,x_c_cat,x_hat_cat,x_d_cat)
...
plot(x_d_cat(1,:),x_d_cat(2,:),'r');
plot(x_c_cat(1,1), x_c_cat(2,1), 'k-o');
text(x_c_cat(1,1)-1.5, x_c_cat(2,1)-1.5, 'x_c', 'FontSize', 18);
plot(x_d_cat(1,1), x_d_cat(2,1), 'k-o');
text(x_d_cat(1,1)+1, x_d_cat(2,1)+1, 'x_d', 'FontSize', 18);
title('Robot navigation with moving target', 'FontSize', 24);
