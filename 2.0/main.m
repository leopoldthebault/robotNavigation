...
radius of target/goal displacement; 'T', 'delta_t' number of iterations to
compute in ODE solution, time-step for ODE solution; 'K' controller gain;
'alpha' correction term coefficient
...
function[] = main() % main(r,x_c,r_d,T,delta_t,K,alpha)
...
% constant time-steps
for t = 1:delta_t:T
    % Target defined as time-varying based on CPU time elapsed - circular
    % trajectory with a period of around 25 seconds
    theta       = 0.25*(cputime - cpu_t);
    x_d         = [r_d*cos(theta);r_d*sin(theta)];
...
nav_plot(W_x,W_y,x_obs,r_obs,x_c_cat,x_hat_cat,x_d_cat);
